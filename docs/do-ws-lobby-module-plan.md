...
- **Phase 2(진행)**: 인라인을 모듈로 1개씩 이관. gomoku는 보류.
  - ✅ **connect4**(커밋 4b273be): startConnect4DO→MultiplayerLobby(3인,min2). handleGameEvent state_update 라우팅, exitGame usingLobby 분기, 기존 DO showResult(mpUI=null) 크래시도 해소. 서버 계약 스모크 통과. **UI 2탭 검증 cocy 대기.**
  - 남음: blackjack/gostop/mahjong/ppingpae
