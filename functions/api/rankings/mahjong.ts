...
async function ensureColumns(DB: D1Database) {
    const cols = ['mahjong_best_score INTEGER DEFAULT 0', 'mahjong_updated_at TEXT'];
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
    ...
    try {
        ...
        const result = await DB.prepare(`
            ...
            WHERE COALESCE(r.mahjong_best_score, 0) > 0
            ORDER BY r.mahjong_best_score DESC, r.mahjong_updated_at ASC
            LIMIT ?
        `).bind(limit).all();
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    ...
    try {
        ...
        await DB.prepare(`
            UPDATE rankings
            SET mahjong_best_score = MAX(COALESCE(mahjong_best_score,0), ?),
                mahjong_updated_at = datetime('now')
            WHERE user_id = ?
