// GET/POST /api/rankings/ppingpae — 최고 승리 점수 랭킹
...
async function ensureColumns(DB: D1Database) {
    const cols = ['ppingpae_best_score INTEGER DEFAULT 0', 'ppingpae_updated_at TEXT'];
    ...
    try {
        ...
        const result = await DB.prepare(`
            SELECT
                ...
                COALESCE(r.ppingpae_best_score, 0) AS best_score,
                r.ppingpae_updated_at AS updated_at
            FROM rankings r
            LEFT JOIN users u ON r.user_id = u.id
            WHERE COALESCE(r.ppingpae_best_score, 0) > 0
            ORDER BY r.ppingpae_best_score DESC, r.ppingpae_updated_at ASC
            LIMIT ?
        `).bind(limit).all();
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    ...
    try {
        ...
        await DB.prepare(`
            UPDATE rankings
            SET ppingpae_best_score = MAX(COALESCE(ppingpae_best_score,0), ?),
                ppingpae_updated_at = datetime('now')
            WHERE user_id = ?
        `).bind(body.score, userId).run();
