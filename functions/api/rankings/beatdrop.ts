// GET/POST /api/rankings/beatdrop — 최고 점수 랭킹 (Beat Drop 리듬게임)
...
async function ensureColumns(DB: D1Database) {
    const cols = ['beatdrop_best_score INTEGER DEFAULT 0', 'beatdrop_updated_at TEXT'];
    ...
    try {
        ...
        const result = await DB.prepare(`
            SELECT
                ...
                COALESCE(r.beatdrop_best_score, 0) AS best_score,
                r.beatdrop_updated_at AS updated_at
            FROM rankings r
            LEFT JOIN users u ON r.user_id = u.id
            WHERE COALESCE(r.beatdrop_best_score, 0) > 0
            ORDER BY r.beatdrop_best_score DESC, r.beatdrop_updated_at ASC
            LIMIT ?
        `).bind(limit).all();
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    ...
    try {
        ...
        const updated = await DB.prepare(`
            UPDATE rankings
            SET beatdrop_best_score = ?, beatdrop_updated_at = datetime('now')
            WHERE user_id = ? AND (COALESCE(beatdrop_best_score, 0) < ?)
        `).bind(body.score, userId, body.score).run();

        const row = await DB.prepare(
            'SELECT beatdrop_best_score FROM rankings WHERE user_id = ?'
        ).bind(userId).first<{ beatdrop_best_score: number }>();

        return Response.json({
            success: true,
            updated: (updated.meta?.changes || 0) > 0,
            bestScore: row?.beatdrop_best_score || body.score,
        }, { headers: CORS });
    } catch (error) {
        return Response.json({ success: false, error: String(error) }, { status: 500, headers: CORS });
    }
