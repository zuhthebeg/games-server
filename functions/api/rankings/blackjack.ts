// GET/POST /api/rankings/blackjack — 최고 순수익 랭킹
...
async function ensureColumns(DB: D1Database) {
    const cols = ['bj_best_profit INTEGER DEFAULT 0', 'bj_updated_at TEXT'];
    ...
    try {
        ...
        const result = await DB.prepare(`
            SELECT
                ...
                COALESCE(r.bj_best_profit, 0) AS best_profit,
                r.bj_updated_at AS updated_at
            FROM rankings r
            LEFT JOIN users u ON r.user_id = u.id
            WHERE COALESCE(r.bj_best_profit, 0) > 0
            ORDER BY r.bj_best_profit DESC, r.bj_updated_at ASC
            LIMIT ?
        `).bind(limit).all();
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    const { DB } = context.env;
    try {
        const body: { userId?: string; nickname?: string; profit: number } =
            await context.request.json();
        const userId = body.userId || context.request.headers.get('x-user-id');
        if (!userId || !body.profit || body.profit <= 0)
            return Response.json({ success: false, error: 'userId and profit required' }, { status: 400, headers: CORS });
        await ensureColumns(DB);
        await ensureUser(DB, userId, body.nickname);
        await DB.prepare('INSERT OR IGNORE INTO rankings (user_id) VALUES (?)').bind(userId).run();
        await DB.prepare(`
            UPDATE rankings
            SET bj_best_profit = MAX(COALESCE(bj_best_profit,0), ?),
                bj_updated_at = datetime('now')
            WHERE user_id = ?
        `).bind(body.profit, userId).run();
