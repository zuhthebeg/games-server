// GET/POST /api/rankings/sudoku — 최고 점수 랭킹
...
async function ensureColumns(DB: D1Database) {
    const cols = ['sudoku_best_score INTEGER DEFAULT 0', 'sudoku_updated_at TEXT'];
    ...
    try {
        ...
        const result = await DB.prepare(`
            SELECT
                ...
                COALESCE(r.sudoku_best_score, 0) AS best_score,
                r.sudoku_updated_at AS updated_at
            FROM rankings r
            LEFT JOIN users u ON r.user_id = u.id
            WHERE COALESCE(r.sudoku_best_score, 0) > 0
            ORDER BY r.sudoku_best_score DESC, r.sudoku_updated_at ASC
            LIMIT ?
        `).bind(limit).all();
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    ...
    try {
        ...
        const updated = await DB.prepare(`
            UPDATE rankings
            SET sudoku_best_score = ?, sudoku_updated_at = datetime('now')
            WHERE user_id = ? AND (COALESCE(sudoku_best_score, 0) < ?)
        `).bind(body.score, userId, body.score).run();

        const row = await DB.prepare(
            'SELECT sudoku_best_score FROM rankings WHERE user_id = ?'
        ).bind(userId).first<{ sudoku_best_score: number }>();

        return Response.json({
            success: true,
            updated: (updated.meta?.changes || 0) > 0,
            bestScore: row?.sudoku_best_score || body.score,
        }, { headers: CORS });
    } catch (error) {
        return Response.json({ success: false, error: String(error) }, { status: 500, headers: CORS });
    }
