// GET/POST /api/rankings/wrongnote — 최고 점수 랭킹
...
async function ensureColumns(DB: D1Database) {
    const cols = ['wrongnote_best_score INTEGER DEFAULT 0', 'wrongnote_updated_at TEXT'];
    ...
    try {
        ...
        const result = await DB.prepare(`
            SELECT
                ...
                COALESCE(r.wrongnote_best_score, 0) AS best_score,
                r.wrongnote_updated_at AS updated_at
            FROM rankings r
            LEFT JOIN users u ON r.user_id = u.id
            WHERE COALESCE(r.wrongnote_best_score, 0) > 0
            ORDER BY r.wrongnote_best_score DESC, r.wrongnote_updated_at ASC
            LIMIT ?
        `).bind(limit).all();
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    ...
    try {
        ...
        const updated = await DB.prepare(`
            UPDATE rankings
            SET wrongnote_best_score = ?, wrongnote_updated_at = datetime('now')
            WHERE user_id = ? AND (COALESCE(wrongnote_best_score, 0) < ?)
        `).bind(body.score, userId, body.score).run();

        const row = await DB.prepare(
            'SELECT wrongnote_best_score FROM rankings WHERE user_id = ?'
        ).bind(userId).first<{ wrongnote_best_score: number }>();

        return Response.json({
            success: true,
            updated: (updated.meta?.changes || 0) > 0,
            bestScore: row?.wrongnote_best_score || body.score,
        }, { headers: CORS });
    } catch (error) {
        return Response.json({ success: false, error: String(error) }, { status: 500, headers: CORS });
    }
