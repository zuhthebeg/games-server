// GET/POST /api/rankings/melodyecho — 최고 점수 랭킹
...
async function ensureColumns(DB: D1Database) {
    const cols = ['melodyecho_best_score INTEGER DEFAULT 0', 'melodyecho_updated_at TEXT'];
    ...
    try {
        ...
        const result = await DB.prepare(`
            SELECT
                ...
                COALESCE(r.melodyecho_best_score, 0) AS best_score,
                r.melodyecho_updated_at AS updated_at
            FROM rankings r
            LEFT JOIN users u ON r.user_id = u.id
            WHERE COALESCE(r.melodyecho_best_score, 0) > 0
            ORDER BY r.melodyecho_best_score DESC, r.melodyecho_updated_at ASC
            LIMIT ?
        `).bind(limit).all();
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    ...
    try {
        ...
        const updated = await DB.prepare(`
            UPDATE rankings
            SET melodyecho_best_score = ?, melodyecho_updated_at = datetime('now')
            WHERE user_id = ? AND (COALESCE(melodyecho_best_score, 0) < ?)
        `).bind(body.score, userId, body.score).run();

        const row = await DB.prepare(
            'SELECT melodyecho_best_score FROM rankings WHERE user_id = ?'
        ).bind(userId).first<{ melodyecho_best_score: number }>();

        return Response.json({
            success: true,
            updated: (updated.meta?.changes || 0) > 0,
            bestScore: row?.melodyecho_best_score || body.score,
        }, { headers: CORS });
    } catch (error) {
        return Response.json({ success: false, error: String(error) }, { status: 500, headers: CORS });
    }
