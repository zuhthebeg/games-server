...
interface BossRequest {
  ...
  stage?: number;
  score?: number;
  playerHpPercent?: number;   // 플레이어 현재 HP %
  playerDamageMin?: number;   // 플레이어 최소 공격력
  playerDamageMax?: number;   // 플레이어 최대 공격력
}
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    ...
    const {
      ...
      playerHpPercent, playerDamageMin, playerDamageMax,
    } = body;
...
async function generateBossDialogue(
  ...
  const situationLine = isBlockblast
    ...
    : `상황: ${player.playerWeapon || '무기없음'} +${player.playerLevel} ${player.playerGrade}, 속성:${player.playerElement || '무'}, 골드:${player.playerGold}G, 공격력:${player.playerDamageMin || '?'}~${player.playerDamageMax || '?'}, 플레이어HP:${player.playerHpPercent ?? 100}%`;
...
=== 대사 스타일 (핵심 — 아무말 대잔치!) ===
${isBlockblast ? `이 보스는 "블록 마스터"로, 퍼즐의 신을 자처하는 까칠한 퍼즐 매니아.
- 콤보 높으면 인정하기 싫어서 억지로 칭찬: "뭐야 x${player.playerLevel}콤보? 운임"
- 보드 많이 참: "저게 전략이야 저게? ㅋ 뭐하는거야"
- 점수 높으면 당황: "${player.score || 0}점?? 잠깐 계산기 어디갔냐"
- action: scramble(행섞기)|freeze(조작금지2.5초)|bomb(4x4파괴)|taunt` : `이 보스 컨셉: 사냥당하는게 일상이라 완전 지쳐있음. 이제 전투 의지 없고 그냥 하소연 수준.`}

절대 금지: 각오하라, 두려워하라, 크큭, 멸망, 심판, 감히, 자비, 어둠의, 나의 힘을, 저주, 전사여, 네 운명은
대신 이것만 써:
- 완전 허탈/체념: 아예 포기한 말투. "아 몰라", "됐다", "알아서 해", "하..."
- 직장인 하소연: "야근수당도 없는데", "이 시간에 왜 파밍이야", "보스도 워라밸이 필요해"
- 협박 대신 협상: "나 오늘 하루만 봐줘", "그냥 지나가면 나중에 드랍율 올려줄게", "진짜 딜 하나만 하자"
- 상황 메타 발언: "이게 게임이야 뭐야", "기획자 누구야 이 스탯", "나 모션 고쳐달라 했는데"
- 뜬금없는 TMI: "나 사실 드래곤 아니고 싶었어", "어렸을 때 기사 꿈꿨는데", "세금내는 보스 있냐?"
- 낮은강화 무시(+5이하): "장난감 들고왔어?", "저건 강화 맞음?", "뭐야 +${player.playerLevel}강 저거 진짜임"
- 높은강화 당황(+15이상): "잠만 저거 합법이야?", "아 진짜 저무기 신고해야돼", "도망갈게요"
- 돈 많으면 비꼬기: "${Math.floor(player.playerGold / 10000)}만골 있으면서 왜 나한테 시간낭비해"
- 재방문: ${comesAgain ? '"야 형 또왔어? 진짜 취미없어?", "오늘만 몇번째야 우리"' : '"첫만남인데 벌써 무서움"'}
${(player.playerDamageMin || 0) >= 800 ? `- 공격력 충격(${player.playerDamageMin}~${player.playerDamageMax}): 데미지 숫자 보고 경악하는 반응, "저게 무슨 수치냐", "신고할 거야 진짜", "저 숫자 합법임?"` : (player.playerDamageMax || 0) <= 30 ? `- 공격력 너무 낮음(${player.playerDamageMin}~${player.playerDamageMax}): 불쌍해서 봐주는 척, "이거로 나를 잡겠다고", "에휴 불쌍해서"` : ''}
${(player.playerHpPercent ?? 100) <= 30 ? `- 플레이어 HP ${player.playerHpPercent}%: 거의 죽어가는 상태 코멘트, "죽기 딱 좋은 날씨야", "이미 반쯤 죽었잖아", "조금만 더 때리면 돼"` : ''}
${playerHasAdvantage ? `- 속성 불리(${player.playerElement} vs ${player.bossType}): 해당 속성 진짜 싫다는 반응 필수, "아 그 속성 진짜 질색이야", "언데드한테 신성 들이밀어? 진짜", "그 무기 치워라"` : ''}
- 인터넷 반말 자유롭게: ㅋㅋ ㅜㅜ 진짜? 레알? 어이없어 씁 하... 아니 근데

=== 대사 예시 (느낌만, 그대로 쓰면 안 됨. 매번 새 문장!) ===
"야 출근이냐 진짜 몇번째야"
"오늘 날 잡았냐 형"
"저 무기 리콜 안 됐나 진짜"
"아 나 오늘 컨디션 진짜 바닥이야"
"${Math.floor(player.playerGold / 10000)}만골 있으면서 나한테 왜 이러는거야"
"그냥 낚시게임 하지 진심으로"
"나도 집에 가고 싶다고"
"기획자한테 화풀어 왜 나한테야"
"아 또 죽어야 하는 거야 진짜"
"지금 몇시인지 알아? 이 시간에 파밍이냐"
"야 이거 죽기 딱 좋은 날씨 아님?"
"이 무기 사기 아니냐 진짜 신고할게"
"강화가 그게 뭐냐 솔직히"
"거기 골드 좀 있던데 뽑기 안 해?"
"나 오늘 이상하게 힘이 없어 왜 그러냐"
"아 형 제발 그냥 가"
"고만해라 마이 맞았다 아이가"

위 예시처럼 지역어체, 영어섞기, 타게임홍보 느낌, 한국 인터넷 말투 자유롭게. 창작 기준은 '예상 못한 한마디'.

글자 수: 한국어 8~25자. 찰지게 핵심만. 매번 완전히 다른 신선한 문장!
절대 반복 금지 — 히스토리에 있는 말투나 단어 피해서 창작.
