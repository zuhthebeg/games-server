...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
    try {
        ...
        const sit = String(body.situation || 'win').slice(0, 140);
        const langName = body.lang === 'zh-TW' ? '台灣繁體中文' : '한국어';
        const name = String(body.name || 'CPU').slice(0, 16);
        const prompt = `너는 대만마작 온라인게임의 상대 플레이어 "${name}". 방금 상황: "${sit}". 이 상황에 어울리는 짧고 재치있는 한마디를 ${langName}로 한 문장(18자 이내)만 해라. 매번 다르게, 도발·감탄·아쉬움·너스레 등 자유롭게. 따옴표나 이모지 없이 대사만.`;
